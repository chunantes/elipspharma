...
/**
 * Classe en charge de tester le service de gestion des arcInvestigateurs.
 * @author Netapsys
 * @version $Revision$ $Date$
 */
public class ArcInvestigateurServiceImplTest {

    /**
     * Service de gestion de arcInvestigateur à tester.
     */
    private PersonneServiceImpl<ArcInvestigateur> service;

    /**
     * Dao mocké.
     */
    private GenericDao<ArcInvestigateur> mockDao;
     ...
     */
    private SaveValidator<ArcInvestigateur> mockSaveValidator;
    ...
    public void init() {
        ...
        this.service = new PersonneServiceImpl<ArcInvestigateur>(this.mockDao);
    ...
    public void testSavePersonneWithValidator() {
        final ArcInvestigateur arcInvestigateur = Mockito.mock(ArcInvestigateur.class);
        final PersonneSuivi suivi = new PersonneSuivi();
        Mockito.when(this.mockDao.reattach(arcInvestigateur)).thenReturn(arcInvestigateur);
        Mockito.when(this.mockSuiviFactory.getInitializedObject()).thenReturn(suivi);
        Mockito.when(arcInvestigateur.getModifs()).thenReturn(new TreeSet<PersonneSuivi>());
        Mockito.when(this.mockDao.save(arcInvestigateur)).thenReturn(arcInvestigateur);
        final Personne result = this.service.save(arcInvestigateur);
        Mockito.verify(this.mockDao).reattach(arcInvestigateur);
        Mockito.verify(this.mockSuiviFactory).getInitializedObject();
        Mockito.verify(this.mockSaveValidator).validate(arcInvestigateur, this.service);
        Mockito.verify(this.mockPasswordEncoderHelper).encodePassword(arcInvestigateur);
        Mockito.verify(this.mockPersonneSaveValidator).validate(arcInvestigateur);
