 ...
 */
public class ForfaitSubProcessorTest extends AbstractEclipseJUnitTest {

    /**
     * Processor à tester.
     */
    private ForfaitSubProcessor subProcessor;
    ...
    public void setUp() {
        ...
        this.subProcessor = new ForfaitSubProcessor();
     ...
     */
    @Test
    public void testProcessWithPatient() {
        final Item item = new Item();
        item.setActe(Acte.DESTRUCTION);
        final Regle regle = new Regle();
        regle.getItems().add(item);
        ...
        Mockito.when(this.counter.process(essai, patient, dateDebut, dateFin)).thenReturn(150);

        Mockito.when(this.counter.process(essai, patient2, dateDebut, dateFin)).thenReturn(51);

        Assert.assertEquals(new BigDecimal(200), this.subProcessor.process(item, regle, essai, dateDebut, dateFin).getMontant());
