...
/**
 * Classe en charge de tester le service de gestion des arcPromoteurs.
 * @author Netapsys
 * @version $Revision$ $Date$
 */
public class ArcPromoteurServiceImplTest {

    /**
     * Service de gestion de arcPromoteur à tester.
     */
    private PersonneServiceImpl<ArcPromoteur> service;

    /**
     * Dao mocké.
     */
    private GenericDao<ArcPromoteur> mockDao;
     ...
     */
    private SaveValidator<ArcPromoteur> mockSaveValidator;
    ...
    public void init() {
        ...
        this.service = new PersonneServiceImpl<ArcPromoteur>(this.mockDao);
    ...
    public void testSavePersonneWithValidator() {
        final ArcPromoteur arcPromoteur = Mockito.mock(ArcPromoteur.class);
        final PersonneSuivi suivi = new PersonneSuivi();
        Mockito.when(this.mockDao.reattach(arcPromoteur)).thenReturn(arcPromoteur);
        Mockito.when(this.mockSuiviFactory.getInitializedObject()).thenReturn(suivi);
        Mockito.when(arcPromoteur.getModifs()).thenReturn(new TreeSet<PersonneSuivi>());
        Mockito.when(this.mockDao.save(arcPromoteur)).thenReturn(arcPromoteur);
        final Personne result = this.service.save(arcPromoteur);
        Mockito.verify(this.mockDao).reattach(arcPromoteur);
        Mockito.verify(this.mockSuiviFactory).getInitializedObject();
        Mockito.verify(this.mockSaveValidator).validate(arcPromoteur, this.service);
        Mockito.verify(this.mockPasswordEncoderHelper).encodePassword(arcPromoteur);
        Mockito.verify(this.mockPersonneSaveValidator).validate(arcPromoteur);
    ...
    public void testGet() {
        ...
        Mockito.when(this.mockDao.get(1L)).thenReturn(arcPromoteur);
        final ArcPromoteur result = this.service.get(1L);
