...
@Transactional
public class E2_ApprovisionnementTest {
    ...
    public void initialize() {
        ...
        Approvisionnement a1 =
            new ApprovisionnementBuilder().withEssai(essai).withPharmacie(pharmacie).withType(TypeMvtStock.APPROVISIONNEMENT).withProduit(medicament)
                    .withConditionnement(conditionnement).withNumLot("FFFF").withDatePeremption(new DateMidnight(2013, 5, 24).toCalendar(EclipseConstants.LOCALE))
                    .withApproApprouve(Boolean.FALSE).withMotifRefus(MotifRefus.PB_TEMPERATURE).withQuantite(10)
                    .withDateReception(new DateMidnight().toCalendar(EclipseConstants.LOCALE)).build();

        Approvisionnement a2 =
            new ApprovisionnementBuilder().withEssai(essai).withPharmacie(pharmacie).withType(TypeMvtStock.APPROVISIONNEMENT).withProduit(medicament)
                    .withConditionnement(conditionnement).withNumLot("FFFFBis").withDatePeremption(new DateMidnight(2013, 5, 25).toCalendar(EclipseConstants.LOCALE))
    ...
    public void testEntreeApppro() {
        ...
        Assert.assertEquals("FFFFBis", l2.getNumLot());
