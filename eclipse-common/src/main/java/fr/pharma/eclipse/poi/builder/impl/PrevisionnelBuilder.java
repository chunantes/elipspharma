 ...
 */
public class PrevisionnelBuilder extends AbstractSheetBuilder implements SheetBuilder, Serializable {
    ...
    protected void createHeaders(final Map<Item, Resultat> datas,
        ...
        sheet.addMergedRegion(new CellRangeAddress(0 + this.getStartRow(), 0 + this.getStartRow(), 4, 7));

        // merge des colonnes de la ligne 2
        // merge de la colonne par patient
        sheet.addMergedRegion(new CellRangeAddress(1 + this.getStartRow(), 1 + this.getStartRow(), 4, 5));
        // merge de la colonne par essai
        sheet.addMergedRegion(new CellRangeAddress(1 + this.getStartRow(), 1 + this.getStartRow(), 6, 7));
