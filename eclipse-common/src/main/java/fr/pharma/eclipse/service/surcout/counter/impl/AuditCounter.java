...
/**
 * Classe en charge de compter le nombre de visites de type audit (interne ou externe) pour un
 * essai.
    ...
    implements ActeCounter, Serializable
{
    ...
    /**
     * Méthode en charge de compter le nombre de visites de type audit (interne ou externe) pour
     * l'essai (ou pour le patient si le patient n'est pas null) dans l'intervalle en paramètre.
     * @param essai L'essai.
                       ...
                       final Calendar dateFin)
    {
        ...
        final int nbAuditsExternes = this.nbVisites(essai,
                                                    patient,
                                                    dateDebut,
                                                    dateFin,
                                                    TypeVisite.AUDIT_EXTERNE);

        final int nbAuditsInternes = this.nbVisites(essai,
                                                    patient,
                                                    dateDebut,
                                                    dateFin,
                                                    TypeVisite.AUDIT_INTERNE);

        return nbAuditsExternes
               + nbAuditsInternes;
    }
    ...
    {
        return super.process(TypeEvenement.VISITE,
                             typeVisite,
                             essai,
                             patient,
                             dateDebut,
