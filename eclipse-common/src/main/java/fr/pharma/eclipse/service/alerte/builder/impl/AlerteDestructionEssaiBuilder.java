 ...
 */
public class AlerteDestructionEssaiBuilder
    implements AlerteBuilder, Serializable
{
    ...
    /**
     * Nombre d'années après la date de visite de clotûre pour l'alerte sur la destruction de
     * l'essai.
     */
                      ...
                      final List<Alerte> alertes)
    {
        ...
        {
            final Calendar dateVisiteCloture = essai.getDetailDates().getCloture();
            if (dateVisiteCloture != null)
            {
                ...
                cal.add(Calendar.YEAR,
                        AlerteDestructionEssaiBuilder.NB_ANNEES);

                if (now.compareTo(cal) > 0)
                {
                    final Alerte alerte = new Alerte(TypeAlerte.DESTRUCTION_ESSAI);
                    alerte.setEssai(essai);
                    alerte.setLibelle(this.messageBuilder
                            .getMessage("alerte.libDestructionEssai",
                                        new Object[]
                                        {AlerteDestructionEssaiBuilder.NB_ANNEES }));
                    alertes.add(alerte);
                }
