...
/**
 * Helper en charge de fournir des méthodes aidant à déduire le début et la fin d'un designable.
...
public class TimeHelper
{
    ...
    public TimeHelper()
    {
        this.mapUniteTemps.put(UniteTemps.SEMAINE,
                               Calendar.WEEK_OF_YEAR);
        this.mapUniteTemps.put(UniteTemps.MOIS,
                               Calendar.MONTH);
        this.mapUniteTemps.put(UniteTemps.JOUR,
    ...
    /**
     * Méthode en charge de retourner le debut minimal de toutes les prescription d'une séquence.
    ...
    public TempsPrescription getDebut(final Collection<PrescriptionType> collection)
    {
        ...
        final Collection coll = this.filterNull(collection,
                                                "debut");

        CollectionUtils.transform(coll,
                                  new GenericTransformer("debut"));
    ...
    /**
     * Filtre les éléments Prescription dont la propriété de type TempsPrescription est non
     * rempli.
                                                    ...
                                                    final String property)
    {
        ...
        return CollectionUtils.select(collection,
                                      new Predicate() {
                                          ...
                                          {
                                              final TempsPrescription t =
                                                  (TempsPrescription) BeanTool
                                                          .getPropriete(object,
                                                                        property);
                                              return t != null
                                                     && t.getNb() != null
    ...
    private SortedSet<TempsPrescription> sortCollectionOfTempsPrescription(final Collection<TempsPrescription> collection)
    {
        ...
        final SortedSet<TempsPrescription> temps =
            new TreeSet<TempsPrescription>(new TempsPrescriptionComparator());
    ...
    /**
     * Méthode en charge de construire le TempsPrescription fin pour une liste de prescription
     * donnée.
     * @param prescriptions Liste de prescriptions.
     * @return Le TempsPrescription représentant la fin la plus élevée pour la liste de
     * prescriptions.
     */
    @SuppressWarnings("all")
    public TempsPrescription getFin(final Collection<PrescriptionType> prescriptions)
    {
        // On ne garde que les prescriptions dont les debut et duree sont correctement renseignés.
        final Collection<PrescriptionType> collection = this.filterNull(prescriptions,
                                                                        "debut");
        final Collection collectionWithoutNull = this.filterNull(collection,
                                                                 "duree");
        CollectionUtils.transform(collectionWithoutNull,
                                  new FinTransformer());

        // Transformation de la collection en collection de fin (TempsPrescription)
        final SortedSet<TempsPrescription> fins =
            this.sortCollectionOfTempsPrescription(collectionWithoutNull);
    ...
    /**
     * Méthode en charge de construire le TempsPrescription fin pour une séquence à partir du
    ...
    {
        sequence.setFin(this.add(sequence.getDebut(),
                                 sequence.getDuree()));
    }

    /**
     * Méthode en charge de retourner le temps prescription de fin maximum du design.
    ...
    public TempsPrescription getDateFinForDesign(final DetailDesign design)
    {
        ...
        CollectionUtils.transform(designables,
                                  new Transformer() {
        ...
        final Object max = Collections.max(designables,
                                           new TempsPrescriptionComparator());
        return (TempsPrescription) max;
    }

    /**
     * Méthode en charge de convertir un UniteTemps en Calendar en fonction de la date courante.
     * @param date La date
                                ...
                                final TempsPrescription temps)
    {
        ...
        c.add(this.mapUniteTemps.get(temps.getUnite()),
              temps.getNb());
        return c;
    }
                                  ...
                                  final TempsPrescription duree)
    {
        if (debut == null
            ...
            || duree.getNb() == null)
        {
            throw new ValidationException("sequence.temps",
                                          new String[]
                                          {"notEmpty", });
        }
