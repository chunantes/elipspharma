 ...
 */
public class ApprovisionnementCounter
    implements ActeCounter, Serializable
{
    ...
    /**
     * Méthode en charge de compter le nombre d'approvisionnements pour l'essai dans l'intervalle
     * en paramètre.
     * @param essai L'essai.
                       ...
                       final Calendar dateFin)
    {
        ...
        // récupération des mouvements de type approvisionnement de l'essai pour la période donnée
        ...
        // le même jour pour une même étude, ne compter qu'un seul mouvement de stock
        final Map<String, MvtStock> listeMouvementsAsMap =
            this.filtreApprovisionnements(listeMouvements);

        return listeMouvementsAsMap.size();
    }

    /**
     * Méthode en charge de compter le nombre d'approvisionnements prévisionnels pour l'essai.
     * @param essai L'essai.
     * @param patient Le patient (si null alors le calcul se fait pour l'essai).
    ...
    private Map<String, MvtStock> filtreApprovisionnements(final List<MvtStock> listeMouvements)
    {
        ...
        for (final MvtStock mvtStock : listeMouvements)
        {
            final String dateMvtAsString =
                ""
                        + mvtStock.getDateCreation().get(Calendar.YEAR)
                        + mvtStock.getDateCreation().get(Calendar.MONTH)
                        + mvtStock.getDateCreation().get(Calendar.DAY_OF_MONTH);
            listeMouvementsAsMap.put(mvtStock.getEssai().getId()
                                             + "-"
                                             + dateMvtAsString,
                                     mvtStock);
        }
