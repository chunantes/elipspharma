 ...
 */
public class ForfaitAnSubProcessor
    extends ForfaitSubProcessor
    implements VariableSubProcessor
{
                            ...
                            final Calendar dateFin)
    {
        ...
        if (visite != null
            && this.getActeCounters().get(item.getActe()) != null)
        {
            ...
            do
            {
                newDateDebut = this.findDateDebut(dateDebut,
                                                  visite,
                                                  i);
                newDateFin = this.findDateFin(newDateDebut,
                                              visite,
                                              dateFin,
                                              i);
                ...
                final int nb = this.getActeCounters().get(item.getActe()).process(essai,
                                                                                  null,
                                                                                  newDateDebut,
                                                                                  newDateFin);
                ...
                result.setNbActes(result.getNbActes()
                                  + r.getNbActes());
                i++;
    ...
    /**
     * Méthode en charge de déterminer la date de début pour le calcul ( 1ere année).
                                   ...
                                   final int i)
    {
        ...
        if (i == 0)
        {
            // si la date de la visite est après la date de début de l'intervalle
                                 ...
                                 final int i)
    {
        ...
        result.add(Calendar.YEAR,
                   1);
        result.add(Calendar.DAY_OF_YEAR,
                   -1);
        if (i == 0)
        {
