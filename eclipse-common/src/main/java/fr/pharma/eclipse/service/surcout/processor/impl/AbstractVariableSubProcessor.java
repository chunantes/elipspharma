 ...
 */
public abstract class AbstractVariableSubProcessor
    implements VariableSubProcessor, Serializable
{
    ...
    {
        final PerimetreCout perimetre = this.findPerimetre(item,
                                                           regle);
        if (null != perimetre)
        {
            if (perimetre.equals(PerimetreCout.PATIENT))
            {
                return this.processByPatient(item,
                                             essai,
                                             regle,
                                             dateDebut,
                                             dateFin);
            ...
            {
                final int nb = this.acteCounters.get(item.getActe()).process(essai,
                                                                             null,
                                                                             dateDebut,
                                                                             dateFin);
                return this.processSubProcessor(regle,
                                                nb);
            }
        }
        return new Resultat();
    ...
    /**
     * Methode en charge de préparer le calcul en vérifiant qu'un acte est bien affecté.
     * @param La regle.
    ...
    /**
     * Méthode calculant le total du montant induit par la règle en paramètre pour chaque patient.
     * @param essai L'essai.
                                      ...
                                      final Calendar dateFin)
    {
        ...
        for (final Patient patient : this.patientService.getAll(criteria))
        {
            final int nb = this.acteCounters.get(item.getActe()).process(essai,
                                                                         ...
                                                                         dateDebut,
                                                                         dateFin);
            final Resultat r = this.processSubProcessor(regle,
                                                        nb);
            value.setMontant(value.getMontant().add(r.getMontant()));
            value.setNbActes(value.getNbActes()
                             + r.getNbActes());
                                      ...
                                      final DonneesPrevision prevision)
    {
        ...
        final Resultat r = this.processSubProcessor(regle,
                                                    nb);
        value.setMontant(value
                .getMontant()
                .add(r.getMontant())
                .multiply(new BigDecimal(essai
    ...
    /**
     * Méthode surchargée par les sous classes en charge d'appliquer l'algorithme spécifique au
     * forfait et à l'unité.
    ...
    /**
     * Méthode en charge de vérifier que l'acte est préent dans le dictionnaire.
     * @param acte L'acte.
     */
    private void checkActe(final Acte acte)
    {

        if (!this.acteCounters.containsKey(acte))
        {
            throw new TechnicalException("L'acte n'est pas pris en compte dans le processus de calcul.");
        }
    }
