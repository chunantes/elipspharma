 ...
 */
public class DosageKgProcessor
    ...
    implements ConseilDispensationProcessor, Serializable
{
    ...
    {
        // Si le poid du patient est null alors il n'est pas possible de calculer la quantité.
    ...
    protected Integer processNbASortir(final ProduitPrescrit produitPrescrit)
    {
        ...
        final double poid =
            Math.ceil(produitPrescrit.getPrescription().getInclusion().getPatient().getPoid());

        return new BigDecimal(poid).multiply(new BigDecimal(nb)).multiply(produitPrescrit
                .getNbUniteDosage()).intValue();
    }
}
