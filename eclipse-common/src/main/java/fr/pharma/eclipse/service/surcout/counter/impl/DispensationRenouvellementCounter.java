 ...
 */
public class DispensationRenouvellementCounter implements ActeCounter, Serializable {
    /**
     * Serial UID.
     */
    ...
    public int process(final Essai essai,
        ...
        // On ne conserve que les dispensations liés à des prescription qui ont
        // été dispensés plus
        ...
        for (final Dispensation d : dispensations) {
            if (d.getPrescription().getDispensations().size() > 1) {
                // première dispensation associée à la prescription, on ne la
                // compte pas.
                if (!disp.containsKey(d.getPrescription().getId())) {
                    disp.put(d.getPrescription().getId(), new Integer(0));
                }
                // pour les autres on compte.
                else {
                    disp.put(d.getPrescription().getId(), new Integer(disp.get(d.getPrescription().getId()) + 1));
                }
            }
        }
