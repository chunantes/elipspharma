...
/**
 * Classe en charge de compter le nombre de mouvements de type "preparation entree" pour un essai
 * ou un patient dans un essai.
 
 * @version $Revision$ $Date$
 */
public abstract class PreparationCounter
    implements ActeCounter, Serializable
{
    ...
    /**
     * Méthode en charge de compter le nombre de preparations pour l'essai (ou pour le patient si
                       ...
                       final boolean sterile)
    {
        if (patient == null)
        {
            // récupération des mouvements de type preparation de l'essai pour la période donnée
            final List<MvtStock> listeMouvements = this.listePreparationsParEssai(essai,
                                                                                  dateDebut,
                                                                                  dateFin,
                                                                                  sterile);
        ...
        else
        {
            ...
            final List<DispensationProduit> listeDispensationProduits =
                this.listeDispensationProduits(dateDebut,
                                               dateFin);

            // on compte le nombre de mvts de type "dispensation" liés au patient
            this.filtreDispensationsPatient(patient,
                                            listeDispensationProduits);
            ...
            if (numLots.size() >= 1)
            {
                // recuperation des mvts de type "preparation entree" liés aux mvts de type
                // "dispensations" récupérés.
                final List<PreparationEntree> listePreparationsEntrees =
                    this.listePreparationsEntrees(numLots,
                                                  sterile);
    ...
    /**
     * Recuperation des mouvements de type "preparations entree" liés aux numeros de lots.
     * @param numLots
     * @param sterile
     */
    ...
    private List<DispensationProduit> listeDispensationProduits(final Calendar dateDebut,
                                                                final Calendar dateFin)
    {
        final DispensationProduitSearchCriteria criteria =
            new DispensationProduitSearchCriteria();
        criteria.setTypeMouvement(TypeMvtStock.DISPENSATION);
        criteria.setDateDebut(dateDebut);
        criteria.setDateFin(dateFin);

        // on recupere les mvts dispensations liées à des mvts de type preparations
    ...
    /**
     * Methode permetant de garder seulement les mouvements "dispensation" liés au patient passé
     * en paramètre
     * @param patient
                                            ...
                                            final List<DispensationProduit> listeDispensationProduits)
    {
        ...
        for (int i = 0; i < listeDispensationProduits.size(); i++)
        {
            ...
            if (dispensationProduit
                    .getDispensation()
                    .getPrescription()
                    .getInclusion()
                    .getPatient()
                    .getId()
                    .intValue() != patient.getId().intValue())
            {
                nbMvtPreparationsClient++;
    ...
    /**
     * Recupere les numeros de lots d'une liste de mouvements de type dispensation.
     * @param listeDispensationProduits
     * @return
     */
     ...
     * @param preparationEntreeStockService Le preparationEntreeStockService à écrire.
     */
