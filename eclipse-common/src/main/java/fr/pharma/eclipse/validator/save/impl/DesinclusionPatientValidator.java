...
/**
 * Validateur appelé afin de vérifier que le patient peut bien être sorti d'un essai.
 
 * @version $Revision$ $Date$
 */
public class DesinclusionPatientValidator
    implements SaveValidator<Patient>, Serializable
{
                         ...
                         final GenericService<Patient> beanService)
    {
        ...
        if (inclusion == null)
        {
            throw new ValidationException("patient.nonInclu",
                                          new String[]
                                          {"error" },
                                          bean);
        }
        ...
        if (!this.getPrescriptions(bean).isEmpty())
        {
            throw new ValidationException("patient.prescription",
                                          new String[]
                                          {"error" },
                                          bean);
        }
        ...
        {
            throw new ValidationException("patient.dispensation",
                                          new String[]
                                          {"error" },
                                          bean);
        }
     ...
     */
    private List<Prescription> getPrescriptions(final Patient bean)
    {
        final PrescriptionSearchCriteria prescriptionCriteria = new PrescriptionSearchCriteria();
        prescriptionCriteria.setPatient(bean);
     ...
     */
    private List<Dispensation> getDispensations(final Patient bean)
    {
        final DispensationSearchCriteria dispensationCriteria = new DispensationSearchCriteria();
        dispensationCriteria.setPatient(bean);
