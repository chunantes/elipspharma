 ...
 */
public class ServiceSaveValidator<BEAN extends Service> implements Serializable, SaveValidator<BEAN> {
    ...
    public void validate(final BEAN bean,
        ...
        if ((result.size() > 0) && !result.get(0).getId().equals(bean.getId())) {
            throw new ValidationException("service.nom", new String[]{"exist" }, bean);
        }

        if ((bean.getSite() == null) && (bean.getPole() == null)) {
            throw new ValidationException("service.pole.site", new String[]{"obligatoire" }, bean);
        }
    }
