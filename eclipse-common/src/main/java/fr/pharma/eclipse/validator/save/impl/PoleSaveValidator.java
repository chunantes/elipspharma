 ...
 */
public class PoleSaveValidator<BEAN extends Pole> implements Serializable, SaveValidator<BEAN> {
    ...
    public void validate(final BEAN bean,
        ...
        if ((result.size() > 0) && !result.get(0).getId().equals(bean.getId())) {
            throw new ValidationException("pole.nom", new String[]{"exist" }, bean);
        }
