...
/**
 * Fabrique d'objets BeanObject devant elle-même créer des sous-objets de
 * l'objet créé.
 * @author Netapsys
 * @version $Revision$ $Date$
 * @param <BEAN> Bean Objet Métier.
 */
public class BeanObjectComplexFactory<BEAN extends BeanObject> extends BeanObjectFactory<BEAN> {
    ...
    private void initBean(final BEAN bean) {
        ...
        for (final Entry<String, BeanObjectWithParentFactory<? extends BeanObject, BEAN>> mapEntry : this.mapFactories.entrySet()) {
            this.initializeProperty(mapEntry.getKey(), mapEntry.getValue(), bean);
        }

        // Appel des fabriques de la map.
        for (final Entry<String, BeanObjectFactory<? extends BeanObject>> mapEntry : this.mapFactoriesWithoutParent.entrySet()) {
            this.initializeProperty(mapEntry.getKey(), mapEntry.getValue(), bean);
        }
    }

    /**
     * Méthode en charge d'initialiser une propriété dans le bean créé par la
     * super-classe.
    ...
    private void initializeProperty(final String beanProperty,
        ...
        BeanTool.setPropriete(bean, beanProperty, subBean);
    }
