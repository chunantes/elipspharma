...
public class ProduitPrescritFillerHelper
{
    ...
    {
        final Collection<JRBeanProduitPrescrit> beanProduits =
            new ArrayList<JRBeanProduitPrescrit>();
        for (final ProduitPrescrit produit : produits)
        {
            ...
            if (produit.getDuree() != null
                && produit.getDuree().getNb() != null
                && produit.getDuree().getUnite() != null)
            {
                jrProduit.setDuree(produit.getDuree().getNb()
                                   + " "
                                   + produit.getDuree().getUnite().getLibelle());
            }
            if (produit.getFrequence() != null
                && produit.getDosage() != null)
            {
                jrProduit.setPosologie(produit.getDosageAsString()
                                       + " "
                                       + produit.getFrequence().toString());
            }
            if (produit.getConditionnement() != null
                && produit.getConditionnement().getVoieAdministration() != null)
            {
                jrProduit.setVoie(produit
                        .getConditionnement()
