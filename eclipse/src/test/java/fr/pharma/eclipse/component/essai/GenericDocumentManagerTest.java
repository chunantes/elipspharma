...
public class GenericDocumentManagerTest
{
     ...
     */
    private static final TypeDocumentEssai TYPE_DOC_MANAGE =
    ...
    public void setUp()
    {
        ...
        this.manager =
            new GenericDocumentEssaiManager<DocumentAutoriteCompetente, DetailAdministratif>(GenericDocumentManagerTest.TYPE_DOC_MANAGE
                    .name());
    ...
    public void testInit()
    {
        ...
        Assert.assertEquals(GenericDocumentManagerTest.TYPE_DOC_MANAGE,
                            this.manager.getTypeDocuments());
        Assert.assertEquals(this.mockedDocService,
                            this.manager.getDocService());
        Assert.assertEquals(this.mockedFactory,
                            this.manager.getFactory());
        Assert.assertEquals(this.mockedFiller,
                            this.manager.getFichierFiller());
        Assert.assertNull(this.manager.getLastDocument());
        Assert.assertEquals(StringUtils.EMPTY,
                            this.manager.getCommentaire());
        Assert.assertNotNull(this.manager.getFichier());
        Assert.assertEquals(0,
                            this.manager.getFichier().getContenu().length);
    }
    ...
    public void testCanCreateDocument()
    {
        ...
        this.manager.getFichier().setContenu(new byte[]
        {0, 1, 0, });
    ...
    public void testCreateDocument()
    {
        ...
        Mockito
                .when(this.mockedFactory.getInitializedObject(detailAdministratif,
                                                              fichier,
                                                              commentaire.trim()))
                .thenReturn(expectedDoc);

        final DocumentEssai actualDoc = this.manager.createDocument(essai);
        Mockito.verify(this.mockedFactory).getInitializedObject(detailAdministratif,
                                                                fichier,
                                                                commentaire.trim());
        Mockito.verify(this.mockedDocService).saveOnDisk(essai,
                                                         expectedDoc,
                                                         fichier);
    ...
    {
        final DocumentAutoriteCompetente expectedLast =
            Mockito.mock(DocumentAutoriteCompetente.class);
        final DocumentAutoriteCompetente oldLast = Mockito.mock(DocumentAutoriteCompetente.class);
    ...
    public void testSetFile()
    {
        ...
        final Answer<Object> answer = new Answer<Object>() {
                ...
                throws Throwable
            {
                ...
                Assert.assertFalse(org.springframework.util.StringUtils.hasText(argFichier
                        .getNom()));
                Assert.assertFalse(org.springframework.util.StringUtils.hasText(argFichier
                        .getTypeFichier()));
                Assert.assertEquals(0,
                                    argFichier.getContenu().length);
                return null;
            }
        };
        Mockito.doAnswer(answer).when(this.mockedFiller).fill(Matchers.any(UploadedFile.class),
                                                              Matchers.any(Fichier.class));
        this.manager.setFile(file);
        Mockito.verify(this.mockedFiller).fill(Matchers.any(UploadedFile.class),
                                               Matchers.any(Fichier.class));
    }
    ...
    public void testSetFileNull()
    {
        ...
        Mockito.verify(this.mockedFiller,
                       Mockito.never()).fill(Matchers.any(UploadedFile.class),
                                             Matchers.any(Fichier.class));
    }
