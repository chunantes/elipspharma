 ...
 */
public class PrescriptionTypeManagerTest extends AbstractEclipseJUnitTest {
    ...
    public void testAddPrescriptionValidate() {
        ...
        Mockito.when(this.mockedHelper.getDebut(Matchers.anyCollection())).thenReturn(new TempsPrescription());
        ...
        Mockito.when(this.mockedSequenceValidator.validateSequence(Matchers.any(PrescriptionType.class), Matchers.any(Sequence.class))).thenReturn(true);
        this.manager.setBean(new PrescriptionType());

        this.manager.addPrescription(event);

        Assert.assertEquals(sequence.getNom(), this.manager.getBean().getSequence().getNom());
    }
    ...
    public void testAddPrescriptionContainedValidate() {
        ...
        Mockito.when(comp.getAttributes()).thenReturn(map);
        Mockito.when(this.mockedHelper.getDebut(Matchers.anyCollection())).thenReturn(new TempsPrescription());
        Mockito.when(this.mockedHelper.getFin(Matchers.anyCollection())).thenReturn(new TempsPrescription());
    ...
    public void testAddPrescriptionContainedValidateKo() {
        ...
        Mockito.when(this.mockedHelper.getFin(Matchers.anyCollection())).thenReturn(new TempsPrescription());
        this.manager.setBean(p);
        Mockito.when(this.mockedSequenceValidator.validateSequence(Matchers.any(PrescriptionType.class), Matchers.any(Sequence.class))).thenReturn(false);
        this.manager.addPrescription(event);
    ...
    public void testIsDosageVisible() {
        ...
        Mockito.when(produit.getType()).thenReturn(TypeProduit.DISPOSITIF_MEDICAL).thenReturn(TypeProduit.PRODUIT_THERAPEUTIQUE).thenReturn(TypeProduit.MEDICAMENT);
    ...
    public void testRemovePrescription() {
        ...
        final UIComponent comp = Mockito.mock(UIComponent.class);
        final Map<String, Object> map = new HashMap<String, Object>();
        map.put("prescriptionToDelete", p);
        Mockito.when(event.getComponent()).thenReturn(comp);
        Mockito.when(comp.getAttributes()).thenReturn(map);
        this.manager.removePrescription(event);

        Assert.assertEquals(0, sequence.getPrescriptions().size());
    }
    ...
    public void testHandleConditionnement() {
        ...
        Assert.assertEquals("test", this.manager.getResumeConditionnement());
        Assert.assertEquals(conditionnement, this.manager.getBean().getConditionnement());
    ...
    public void testBuildResume() {
        this.manager.setBean(new PrescriptionType());
        Mockito.when(this.resumeHelper.buildResume(Matchers.any(PrescriptionType.class))).thenReturn("test");
    ...
    public void testEditPrescription() {
        ...
        attributes.put("prescriptionToEdit", p);
        Mockito.when(event.getComponent()).thenReturn(component);
        Mockito.when(this.conditionnementHelper.buildResume(Matchers.any(Conditionnement.class))).thenReturn("test");
