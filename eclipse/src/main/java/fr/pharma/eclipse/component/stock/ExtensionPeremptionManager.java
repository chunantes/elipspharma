 ...
 */
public class ExtensionPeremptionManager extends BeanListManager<Approvisionnement> {
    /**
     * Serial ID.
     */
    ...
    public void handleSelectPharmacie(final AjaxBehaviorEvent event) {
        ...
        if (this.searchCriteria.getEssai() != null) {
            this.setProduits(this.produitService.getProduits(this.searchCriteria.getEssai(), this.searchCriteria.getPharmacie()));
        } else {
    ...
    /**
     * Méthode en charge de déterminer si le délai d'alerte avant la date de
     * péremption est atteint pour le produit.
    ...
    public Boolean delaiAlerteAvantExpirationAtteint(final Approvisionnement appro) {
        ...
        if (datePeremption != null) {
            // Récupération du délai d'alerte avant la date de péremption pour
            // un produit
            // Délai en jours
            final Produit produit = appro.getProduit();
            final Integer delaiAlerte = produit.getDetailLogistique().getDelaiAlerteAvtDateExpiration();

            final Calendar calendar = Calendar.getInstance(EclipseConstants.LOCALE);
            calendar.setTime(datePeremption.getTime());
            if (delaiAlerte != null) {
                calendar.add(Calendar.DAY_OF_MONTH, -delaiAlerte);
            }
    ...
    public void addDocAppro() {
        ...
        BeanTool.setPropriete(this.getBeanSelected(), TypeDocumentStock.APPRO.getPropriete(), doc);
    }
    ...
    public void delDocAppro(final ActionEvent event) {
        final DocumentStock doc = (DocumentStock) BeanTool.getPropriete(this.getBeanSelected(), TypeDocumentStock.APPRO.getPropriete());

        BeanTool.setPropriete(this.getBeanSelected(), TypeDocumentStock.APPRO.getPropriete(), null);
