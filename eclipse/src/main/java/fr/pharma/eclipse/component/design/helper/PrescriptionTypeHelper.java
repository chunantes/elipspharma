...
public class PrescriptionTypeHelper
{
    ...
    {"debut", "debut.unite", "debut.nb", "duree", "duree.unite", "duree.nb", "frequence",
     "frequence.nbFrequence", "frequence.typeRegularite", "conditionnement",
     "conditionnement.modePrescription" };
    ...
    public String buildResume(final PrescriptionType prescription)
    {
        ...
        // si le type de frequence est different de "une fois" alors, on verifie que l'unite de
        // frequence n'est pas nulle
        proprietesAVerifier = this.majProprietesAverifier(prescription);

        if (this.verifyNotNull(prescription,
                               proprietesAVerifier))
        {
            buff.append("Prescription : ");

            // si mode prescription par dose.
            this.buildResumeAjoutDose(prescription,
                                      buff);
            ...
            if (prescription
                    .getConditionnement()
                    .getModePrescription()
                    .equals(ModePrescription.NUM_TRAITEMENT))
            {
                buff.append("Par numéro de traitement, ");
            }

            // Si mode de prescription galénique
            buildResumeAjoutPrescriptionGalenique(prescription,
                                                  buff);

            // Si mode de prescription conditionnement primaire
            this.buildResumeAjoutConditionnementPrimaire(prescription,
                                                         buff);

            // fréquence
            this.buildResumeAjoutFrequence(prescription,
                                           buff);

            // debut
            buff.append("à partir de ")
                    .append(prescription.getDebut().getUnite().getLibelleCourt())
                    .append(prescription.getDebut().getNb())
                    .append(EclipseConstants.SPACE);

            // durée
            buff.append("pendant ")
                    .append(prescription.getDuree().getNb())
                    .append(EclipseConstants.SPACE)
                    .append(prescription.getDuree().getUnite().getLibPluriel());
                                                       ...
                                                       final StringBuffer resume)
    {
        if (prescription
                .getConditionnement()
                .getModePrescription()
                .equals(ModePrescription.FORME_GALENIQUE)
            && prescription.getNbUniteDosage() != null)
        {
            resume.append(prescription.getNbUniteDosage())
                    .append(EclipseConstants.SPACE)
                    .append("comprimés/gélules, ");
        }
    }
    /**
                                                         ...
                                                         final StringBuffer resume)
    {
        if (prescription
                .getConditionnement()
                .getModePrescription()
                .equals(ModePrescription.CONDITIONNEMENT_PRIMAIRE)
            && prescription.getNbUniteDosage() != null)
        {
            resume.append(prescription.getNbUniteDosage())
                    .append(EclipseConstants.SPACE)
                    .append(prescription.getConditionnement().getUnitePrescription())
    ...
    {
        final String[] props =
        {"conditionnement.unitePrescription", "nbUniteDosage", "dosage" };
        ...
        if (this.verifyNotNull(prescription,
            ...
            && prescription
                    .getConditionnement()
                    .getModePrescription()
                    .equals(ModePrescription.DOSE)
            || prescription
                    .getConditionnement()
                    .getModePrescription()
                    .equals(ModePrescription.DOSE_KG)
            || prescription
                    .getConditionnement()
                    .getModePrescription()
                    .equals(ModePrescription.DOSE_SURFACE))
        {
            resume.append(prescription.getNbUniteDosage())
                    .append(" fois ")
    ...
    private void buildResumeAjoutFrequence(final PrescriptionType prescription,
                                           final StringBuffer resume)
    {
        resume.append(prescription.getFrequence().getNbFrequence())
                .append(" fois")
                .append(EclipseConstants.SPACE);
        if (!prescription.getFrequence().getTypeRegularite().equals(TypeRegularite.FOIS))
        {
            resume.append(prescription.getFrequence().getTypeRegularite())
                    .append(EclipseConstants.SPACE);
        }
        if (prescription.getFrequence().getTypeRegularite().equals(TypeRegularite.TOUS_LES))
        {
            resume.append(prescription.getFrequence().getNbUniteTempsFrequence())
                    .append(EclipseConstants.SPACE);
        }
        if (!prescription.getFrequence().getTypeRegularite().equals(TypeRegularite.FOIS))
        {
            resume.append(prescription.getFrequence().getUniteFrequence().getLibPluriel())
                    .append(EclipseConstants.SPACE);
        }
    }
    /**
    ...
    {
        String[] proprietesAVerifier;
        if (prescription.getFrequence().getTypeRegularite() != null
            && !prescription.getFrequence().getTypeRegularite().equals(TypeRegularite.FOIS))
        {
            // on ajoute l'unite de frequence à la liste des propriétés à verifier
            final List<String> proprietesAsArrayList =
                new ArrayList<String>(Arrays.asList(this.proprietes));
            proprietesAsArrayList.add(new String("frequence.uniteFrequence"));
            proprietesAVerifier =
                proprietesAsArrayList.toArray(new String[proprietesAsArrayList.size()]);
        }
        else
        {
    ...
    /**
     * Méthode en charge de vérifier que les propriétés dans proprerties ne sont pas nulles.
                                  ...
                                  final String[] proprietes)
    {
        ...
        {
            if (BeanTool.getPropriete(prescription,
                                      s) == null)
            {
                return false;
            }
