 ...
 */
public class AutoSaisieNumTraitement implements Serializable {
    ...
    /**
     * Méthode en charge de gérer le traitement de la saisie automatique des
     * numéros de traitements.
    ...
    public void handle(final List<NumTraitement> numsTraitements) {
        if (this.launchSaisieAuto(numsTraitements)) {
            ...
            if (StringUtils.isNotEmpty(suffixe)) {
                final String prefixe = numTraitementModele.substring(0, numTraitementModele.lastIndexOf(suffixe));
                ...
                for (int i = 1; i < numsTraitements.size(); i++) {
                    ...
                    sb.append(prefixe);
                    sb.append(this.complete(suffixe, String.valueOf(++numberSuffixe)));
     ...
     * @param nb Nombre.
     * @return Nombre complété avec des zéros devant pour que la taille du
     * suffixe soit bonne.
     */
    ...
    /**
     * Méthode en charge de détecter le lancement du remplissage automatique des
    ...
    protected boolean launchSaisieAuto(final List<NumTraitement> numsTraitements) {
        ...
        // On lance la saisie automatique si le premier élément est rempli et
        // les autres vides.
        int index = 0;
        for (final NumTraitement numTraitement : numsTraitements) {
            // Traitement de l'élément index 0
            if ((index == 0) && (StringUtils.isEmpty(numTraitement.getNumTraitement()))) {
                launch = false;
                break;
            }
