 ...
 */
public class EssaiManager extends BeanManager<Essai> {
	...
	/**
	 * Dictionnaire des managers des collections de documents de l'essai selon
	 * leur type.
	 */
	@SuppressWarnings("rawtypes")
	...
	public EssaiManager(final GenericService<Essai> service) {
		super(service);
		this.log.debug("Construction de l'objet EssaiManager : " + this);
	}
	/** ************************ **/
	 ...
	 * 
	 * @param event
	 *            Evénement remonté via la couche IHM.
	 */
	public void handleSelectPromoteur(final SelectEvent event) {
		updateTypePromoteur((Promoteur) event.getObject());
	}
	 ...
	 * 
	 * @param promoteur
	 *            Promoteur sélectionné.
	 */
	void updateTypePromoteur(final Promoteur promoteur) {
		if ((promoteur == null) || (promoteur.getType() == null)) {
			return;
		}
		getBean().setTypePromoteur(promoteur.getType());
	}
	...
	public DataModel<Habilitation> getGroupePromoteurs() {
		reattach();
		return getHelper()
				.returnAsDataModel(this.contactsManager.getGroupeHabilitations(getBean(), GroupeContacts.PROMOTEURS));
	}
	...
	public DataModel<Habilitation> getGroupeInvestigateurs() {
		return getHelper().returnAsDataModel(
				this.contactsManager.getGroupeHabilitations(getBean(), GroupeContacts.INVESTIGATEURS));
	}
	...
	public DataModel<Habilitation> getGroupeDRC() {
		return getHelper()
				.returnAsDataModel(this.contactsManager.getGroupeHabilitations(getBean(), GroupeContacts.DRC));
	}
	...
	public DataModel<Habilitation> getGroupePharmaciens() {
		return getHelper()
				.returnAsDataModel(this.contactsManager.getGroupeHabilitations(getBean(), GroupeContacts.PHARMACIENS));
	}
	...
	public boolean hasInvestigateurPrincipal() {
		return this.contactsManager.hasInvestigateurPrincipal(getBean());
	}
	...
	public void disableContacts() {
		this.contactsManager.disableContacts(getBean());
	}
	...
	/**
	 * Méthode en charge de lancer le calcul des contacts sélectionnables pour
	 * l'ajout.
	 * 
	 * @param typeContactToAdd
	 *            Type de contact à ajouter.
	...
	public void calculatePersons(final TypeContact typeContactToAdd) {
		this.contactsManager.setTypeContactToAdd(typeContactToAdd);
		this.contactsManager.initSelectableContacts(getBean());
	}
	...
	public void validerAjoutContacts() {
		this.contactsManager.ajouterContacts(getBean());
	}
	 ...
	 * 
	 * @param selectedHabilitation
	 *            L'habilitation sélectionnée.
	 */
	...
	/**
	 * Méthode d'initialisation des derniers commentaires où les types à
	 * initialiser sont spécifiés.
	 * 
	 * @param types
	 *            Liste des noms des types de commentaires à initialiser.<br>
	 *            S'il n'y a pas de paramètres, appelle la méthode
	 *            initLastCommentaires sans paramètre.
	 */
	@SuppressWarnings("rawtypes")
	public void initLastCommentaires(final String... types) {
		...
		final List<GenericCommentaireManager> managers = (List<GenericCommentaireManager>) CollectionUtils
				.select(this.commentairesManagers.values(), predicate);
		this.initLastCommentaires(managers);
	}

	/**
	 * Méthode d'initialisation des derniers commentaires d'une liste de
	 * managers.
	 * 
	 * @param managers
	...
	private void initLastCommentaires(final Collection<GenericCommentaireManager> managers) {
		for (final GenericCommentaireManager manager : managers) {
			manager.initLastCommentaire(getBean());
		}
	}
	 ...
	 *            Type du commentaire à ajouter.
	 */
	@SuppressWarnings("rawtypes")
	public void ajouterCommentaire(final TypeCommentaireEssai typeCommentaire) {
		final GenericCommentaireManager commentManager = this.getCommentaireManager(typeCommentaire);
		...
		final CommentaireEssai commentaire = commentManager.createCommentaire(getBean());
		getHelper().addToCollection(getBean(), typeCommentaire.getCommentairesPropertyFromEssai(), commentaire);
		commentManager.setLastCommentaire(commentaire);
	}

	/**
	 * Méthode en charge de récupérer le commentaire le premier commentaire de
	 * l'ensemble.
	 * 
	 * @param typeCommentaire
	 *            Type du commentaire.
	 * @return Le bean CommentaireEssaiRecherche le plus récent (null si aucun
	 *         commentaire).
	 */
	@SuppressWarnings("rawtypes")
	...
	/**
	 * Méthode de récupération d'un manager de commentaires d'un certain type
	 * dans la map.
	 * 
	 * @param typeCommentaireName
	 *            Nom du type de commentaire {@link TypeCommentaireEssai}.
	 * @return Le GenericCommentaireManager correspondant au type de commentaire
	 *         dans la map. Null si inexistant.
	 */
	@SuppressWarnings("rawtypes")
	...
	private GenericCommentaireManager getCommentaireManager(final TypeCommentaireEssai typeCommentaire) {
		final GenericCommentaireManager commentManager = this.commentairesManagers.get(typeCommentaire.name());
		if ((commentManager == null) && this.log.isDebugEnabled()) {
			this.log.error(new StringBuilder("[getManager] ").append("Aucun manager de commentaires ")
					.append("n'est défini dans la map pour le type ").append(typeCommentaire.name())
					.append(" : le commentaire n'est pas traité.").toString());
		}
	 ...
	 *            Type de document.
	 * @param collection
	 *            Manager d'une collection ou non.
	...
	public GenericDocumentEssaiManager getDocumentManager(final TypeDocumentEssai typeDocument,
		...
		if ((manager == null) && this.log.isDebugEnabled()) {
			this.log.error(new StringBuilder("[getDocumentManager] ")
					...
					.append(typeDocument.name()).append(" : le document n'est pas traité.").toString());
		}
		return manager;
	}

	/**
	 * Méthode d'initialisation des derniers documents de chacun des managers de
	 * documents.
	 */
	@SuppressWarnings("rawtypes")
	public void initLastDocuments() {
		for (final GenericDocumentEssaiManager manager : this.documentsCollectionManagers.values()) {
			manager.initLastDocument(getBean());
		}
	}
	...
	public DocumentEssai getLastDocument(final TypeDocumentEssai typeDocument) {
		@SuppressWarnings("rawtypes")
		final GenericDocumentEssaiManager manager = getDocumentManager(typeDocument, true);
		if (manager == null) {
			return null;
		}
	 ...
	 * 
	 * @param typeDocument
	 *            Type du document d'essai à ajouter.
	 * @param collection
	 *            Document d'une collection ou non
	 */
	public void ajouterDocument(final TypeDocumentEssai typeDocument, final boolean collection) {
		@SuppressWarnings("rawtypes")
		final GenericDocumentEssaiManager docManager = getDocumentManager(typeDocument, collection);
		if (docManager == null) {
			return;
		}
		...
		final DocumentEssai doc = docManager.createDocument(getBean());

		if (collection) {
			getHelper().addToCollection(getBean(), typeDocument.getDocumentsPropertyFromEssai(), doc);

			docManager.setLastDocument(doc);
		} else {
			// parsing de la propriété représentant le document.
			final String propertyDocument = StringUtils.substringAfterLast(typeDocument.getDocumentsPropertyFromEssai(),
					".");
			// parsing de la propriété du parent du document à partir de
			// l'essai.
			final String parentProperty = StringUtils.substringBeforeLast(typeDocument.getDocumentsPropertyFromEssai(),
					".");
			BeanTool.setPropriete(BeanTool.getPropriete(getBean(), parentProperty), propertyDocument, doc);
		}
	}
	...
	public void supprimerDocument(final TypeDocumentEssai typeDocument) {
		final DocumentEclipse doc = (DocumentEssai) BeanTool.getPropriete(getBean(),
		...
		BeanTool.setPropriete(BeanTool.getPropriete(getBean(), parentProperty), propertyDocument, null);
	...
	public void initLastModifs() {
		Assert.notNull(getBean(), "Le bean du manager doit être non nul.");
		this.dicoDernieresModifs.init(getBean());
	}

	/**
	 * Méthode pour indiquer si l'essai possède une dernière modification d'un
	 * certain type.
	 * 
	 * @param ongletEssaiName
	 *            Type de la modification, correspondant au nom à la valeur de
	 *            l'énumération TypeHistoriqueEssai correspondante.
	 * @return true ssi l'essai possède une dernière modification pour le type
	 *         donné.
	 */
	public boolean hasLastModif(final String ongletEssaiName) {
		return this.dicoDernieresModifs.hasDerniereModif(TypeHistoriqueEssai.valueOf(ongletEssaiName));
	}
	...
	public Suivi getLastModif(final String ongletEssaiName) {
		return this.dicoDernieresModifs.getDerniereModif(TypeHistoriqueEssai.valueOf(ongletEssaiName));
	}
	 ...
	 * 
	 * @param event
	 *            Evénement remonté par le composant primeFaces.
	 */
	public void onOngletChange(final TabChangeEvent event) {
		final String tabId = event.getTab().getId();
		if (!StringUtils.contains(this.idOngletVisites, tabId)
				&& StringUtils.isNotEmpty(TypeHistoriqueEssai.valueOf(tabId).getModifsPropertyFromEssai()))
	 ...
	 * 
	 * @param tabId
	 *            Identifiant de l'onglet à sélectionner.
	 */
	public void selectOngletCourant(final String tabId) {
		try {
			setIndexOngletCourant(TypeHistoriqueEssai.valueOf(tabId).getIndexIHM());
			...
			if (this.log.isDebugEnabled()) {
				this.log.debug(new StringBuilder("[onOngletChange] ")
						...
						.append("pas de correspondance avec TypeHistoriqueEssai pour l'id ").append(tabId).append(".")
						.toString());
			}
		}
	}
	...
	public void resetChangementEtat() {
		setEtatChgtEtat(getBean().getEtat());
		setCommentaireChgtEtat(null);
	}
	...
	public void ajouterDetailEtatEssai() {
		...
		if (getBean().getEtat().equals(EtatEssai.EN_ATTENTE_MISE_EN_PLACE)
				|| getBean().getEtat().equals(EtatEssai.MISE_EN_PLACE)
				|| getBean().getEtat().equals(EtatEssai.EN_EVALUATION)) {
			wasInactivate = true;
		}
		...
		// Mise à jour de la date d'activation de l'essai si l'essai passe de
		// désactivé à activé
	...
	/**
	 * Méthode en charge de définir si les champs de surcouts previsionnel sont
	 * éditables.
	 * 
	 * @return true si les champs sont editables.
	 */
	public boolean isSurcoutPrevisionnelEditable() {
		return getBean().getEtat().equals(EtatEssai.EN_EVALUATION)
				|| getBean().getEtat().equals(EtatEssai.EN_ATTENTE_MISE_EN_PLACE)
				|| getBean().getEtat().equals(EtatEssai.MISE_EN_PLACE);
	}
	...
	public void save() {
		try {
			setBean(((EssaiService) getService()).save(getBean(), this.idOngletVisites));
			this.facesUtils.addMessage(FacesMessage.SEVERITY_INFO, "essai.enregistrement.ok");
			...
			this.facesUtils.addMessage(FacesMessage.SEVERITY_ERROR, strToResolve.toString());
		}
		this.idOngletVisites = StringUtils
				.substringAfterLast(StringUtils.substringBeforeLast(this.idOngletVisites, ","), ",") + ",";
	}

	public Produit getProduitByName(final Produit produit) {
		@SuppressWarnings("unchecked")
		final Produit result = (Produit) CollectionUtils.find(
				(Collection<Produit>) BeanTool.getPropriete(getBean().getDetailProduit(),
						produit.getType().getCollection()),
				new GenericPredicate("denomination", produit.getDenomination()));
		return result;
	}
	 ...
	 * 
	 * @param dicoDernieresModifs
	 *            le dicoDernieresModifs à écrire.
	 */
	 ...
	 * 
	 * @param commentairesManagers
	 *            le commentairesManagers à écrire.
	 */
	@SuppressWarnings("rawtypes")
	 ...
	 * 
	 * @param indexOngletCourant
	 *            le indexOngletCourant à écrire.
	 */
	 ...
	 * 
	 * @param documentsCollectionManagers
	 *            le documentsCollectionManagers à écrire.
	 */
	@SuppressWarnings("rawtypes")
	public void setDocumentsCollectionManagers(
			final SortedMap<String, GenericDocumentEssaiManager> documentsCollectionManagers) {
	 ...
	 * 
	 * @param documentManagers
	 *            le documentManagers à écrire.
	 */
	@SuppressWarnings("rawtypes")
	 ...
	 * 
	 * @param evenementSelected
	 *            Le evenementSelected à écrire.
	 */
	 ...
	 * 
	 * @param commentaireChgtEtat
	 *            Le commentaireChgtEtat à écrire.
	 */
	 ...
	 * 
	 * @param idOngletVisites
	 *            Le idOngletVisites à écrire.
	 */
	...
	public void setBean(final Essai essai) {
		this.log.debug("[setBean] EssaiManager : " + this + ", Essai : " + essai);
		super.setBean(essai);
	}
