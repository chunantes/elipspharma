 ...
 */
public class ProduitListManager extends BeanListManager<Produit> {
     ...
     * collection correspondante dans le detailProduit.
     */
    ...
    /**
     * Map contenant les informations de redirection en fonction du type de
     * personne.
     */
    public static final Map<TypeProduit, String[]> INFOS_REDIRECT = new TreeMap<TypeProduit, String[]>();

    static {
        ProduitListManager.INFOS_REDIRECT.put(TypeProduit.MEDICAMENT, new String[]{"editMedicament", "idMedicament" });
        ProduitListManager.INFOS_REDIRECT.put(TypeProduit.DISPOSITIF_MEDICAL, new String[]{"editDispositifMedical", "idDm" });
        ProduitListManager.INFOS_REDIRECT.put(TypeProduit.PRODUIT_THERAPEUTIQUE, new String[]{"editProduitTherapeutique", "idProduitThera" });
    }
     ...
     */
    public ProduitListManager(final SearchCriteria searchCriteria) {
    ...
    public void remove(final Essai essai) {
        ...
        for (final Produit p : this.getBeansSelected()) {
            ...
            ((SortedSet) BeanTool.getPropriete(essai.getDetailProduit(), this.typeProduitDictionary.get(p.getType()))).remove(p);
    ...
    /**
     * Méthode en charge de dupliquer les produits sélectionnés de l'essai en
     * paramètre.
     * @param essai L'essai.
     */
    ...
    public void dupliquer(final Essai essai) {
        ...
        for (final Produit p : this.getBeansSelected()) {
            ...
            ((SortedSet) BeanTool.getPropriete(essai.getDetailProduit(), this.typeProduitDictionary.get(p.getType()))).add(clone);
            p.setSelected(false);
    ...
    /**
     * Méthode en charge de supprimer le produit en création lors d'une
     * annulation.
     * @param produit Le produit.
     */
    public void removeProduitOnCancel(final Produit produit) {
        if (produit.getId() == null) {
            ((SortedSet) BeanTool.getPropriete(produit.getDetailProduit(), this.typeProduitDictionary.get(produit.getType()))).remove(produit);
        }
    }
     ...
     * produit <br />
     * à partir de la sélection d'une ligne dans le tableau de résultats de
     * Produit. <br />
    ...
    /**
     * Méthode en charge de gérer la redirection vers la page d'ajout d'un
     * produit <br />
    ...
    public void handleRedirect(final TypeProduit type) {
        super.redirect(ProduitListManager.INFOS_REDIRECT.get(type)[0], ProduitListManager.INFOS_REDIRECT.get(type)[1]);
    }
