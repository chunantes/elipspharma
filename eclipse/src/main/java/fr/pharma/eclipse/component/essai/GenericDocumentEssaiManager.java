...
/**
 * Classe générique en charge de gérer un type de documents sur un essai
 * clinique.
 ...
 */
public class GenericDocumentEssaiManager<DOC extends DocumentEssai, PARENT extends BeanObject> implements Serializable {
     ...
     * @return Un nouveau DocumentEssai. Le fichier physique est créé sur le
     * disque.
     */
    @SuppressWarnings("unchecked")
    public DOC createDocument(final Essai essai) {
        // Document BDD.
        final PARENT parent = (PARENT) BeanTool.getPropriete(essai, this.typeDocuments.getDocumentsParentPropertyFromEssai());
        final DOC doc = this.factory.getInitializedObject(parent, this.fichier, this.commentaire.trim());

        // Document Physique.
        this.docService.saveOnDisk(essai, doc, this.fichier);
    ...
    public void initLastDocument(final Essai essai) {
        ...
        final SortedSet<DOC> documents = (SortedSet<DOC>) BeanTool.getPropriete(essai, this.typeDocuments.getDocumentsPropertyFromEssai());
