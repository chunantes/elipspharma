    ...
    extends BeansManager<Patient>
{
    ...
    {"ROLE_ARC_INVESTIGATEUR", "ROLE_ARC_PROMOTEUR" };
     ...
     */
    public PatientsManager(final SearchCriteria searchCriteria)
    {
        super(searchCriteria);
    }

    /**
     * Méthode appelé lorsqu'un essai est sélectionné sur l'écran de prescription. Il ajoute au
     * criteria l'essai.
    ...
    /**
     * Pour l'export : retourne le numero d'inclusion et la date d'inclusion du patient en
     * paramètre.
    ...
    public String getNumDateInclusion(final Patient patient)
    {
        ...
        if (inclusion != null)
        {
            return inclusion.getNumInclusion()
                   + " - "
                   + Utils.formatDate(inclusion.getDateInclusion().getTime(),
    ...
    public String getDateInclusion(final Patient patient)
    {
        ...
        if (inclusion != null)
        {
            return Utils.formatDate(inclusion.getDateInclusion().getTime(),
                                    EclipseConstants.PATTERN_SIMPLE);
        }
        else
        {
    ...
    public String getEssai(final Patient patient)
    {
        ...
        if (inclusion != null)
        {
            return inclusion.getEssai().getPromoteur().getRaisonSociale()
                   + " - "
                   + inclusion.getEssai().getCodePromoteur()
                   + " - "
                   + inclusion.getEssai().getNom();
        }
        else
        {
            return EclipseConstants.NON_APPLICABLE;
