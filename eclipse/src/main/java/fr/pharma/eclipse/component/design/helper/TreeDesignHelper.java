...
/**
 * Classe de helper concernant la représentation sous forme d'arbre des s d'une
 * elements du design (bras, sous-bras, sequence).
 * @author Netapsys
 * @version $Revision$ $Date$
 */
public class TreeDesignHelper implements Serializable {
    /**
     * Serial ID.
     */
    ...
    /**
     * Méthode en charge de construire les données de l'arbre des éléments du
     * design d'un essai.
     * @param essai Essai.
     * @return Arbre.
     */
    public TreeNode buildTree(final DetailDesign design) {
        ...
        for (final Designable designable : designRoot) {
            final TreeNode nodeParent = new DefaultTreeNode(designable, root);
            this.handleNoeuds(nodeParent, designable);
        }
        return root;
    }
     ...
     * @param designable Bean Designable de référence dans l'arbre pour gérer
    ...
    public String calculateNodesToExpand(final TreeNode tree,
        ...
        for (final Iterator<TreeNode> iterator = tree.getChildren().iterator(); iterator.hasNext();) {
            result = this.calculateIdTreeNode(iterator.next(), designable, String.valueOf(index));
    ...
    private String calculateIdTreeNode(final TreeNode node,
        ...
        if (((Designable) node.getData()).getNomComplet().equals(designable.getNomComplet())) {
            ...
            for (final Iterator<TreeNode> iterator = node.getChildren().iterator(); iterator.hasNext();) {
                ...
                rowIndex++;
                result = this.calculateIdTreeNode(iterator.next(), designable, nodeRowKey);
                if (StringUtils.isNotEmpty(result)) {
                    break;
                }
            }
    ...
    protected String decomposeNodes(final String input) {
        ...
        if (StringUtils.isNotEmpty(tmp)) {
            result += TreeDesignHelper.NAME_TREE_DESIGN + tmp + EclipseConstants.COMMA;
