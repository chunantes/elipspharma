 ...
 */
public class SortieFileHelper implements Serializable {
    /**
     * Serial ID.
     */
    ...
    /**
     * Méthode en charge d'ajouter les documents enregistrés sur sortie sur la
    ...
    public void addDocumentsSortie(final TypeMvtStock typeSortie,
                                   ...
                                   final UploadedFile fileRetourPromoteur) throws IOException {
        final List<? extends MvtStock> mvts = resultSortie.getMvts();

        // Type de sortie Destruction et Fichier de destruction saisi
        if ((TypeMvtStock.DESTRUCTION.equals(typeSortie)) && (fileDestruction != null) && (fileDestruction.getBytes().length > 0)) {
            for (final MvtStock mvt : mvts) {
                ...
                BeanTool.setPropriete(mvt, TypeDocumentStock.DESTRUCTION.getPropriete(), doc);
            }
        ...
        else if ((TypeMvtStock.RETOUR_PROMOTEUR.equals(typeSortie)) && (fileRetourPromoteur != null) && (fileRetourPromoteur.getBytes().length > 0)) {
            for (final MvtStock mvt : mvts) {
                ...
                BeanTool.setPropriete(mvt, TypeDocumentStock.RETOUR_PROMOTEUR.getPropriete(), doc);
            }
            // Sauvegarde des destructions
