...
public class ConditionnementHelper {
     ...
     */
    String[] propriete = {"uniteGestion", "forme", "voieAdministration", };

    /**
     * Méthode en charge de construire le résumé si tous les champs nécessaires
     * sont settés.
    ...
    public String buildResume(final Conditionnement conditionnement) {
        ...
        for (final String s : this.propriete) {
            if (BeanTool.getPropriete(conditionnement, s) == null) {
                sette = false;
            }
        }
        ...
        if (sette) {
            flacon =
                conditionnement.getUniteGestion().getType().equals(TypeUniteGestion.VOLUME) && conditionnement.getForme().equals(FormeConditionnement.CONDITIONNEMENT_PRIMAIRE);

            // si on est dans le cas d'un flacon on vérifie la contenance.
            if (flacon) {
                if ((conditionnement.getContenance() == null) || (conditionnement.getUniteContenance() == null)) {
                    sette = false;
                }
            }
        }
        ...
        if (sette) {
            if (!flacon) {
                result.append(conditionnement.getUniteGestion().getLibelle()).append(EclipseConstants.SPACE);
                ...
                if ((conditionnement.getDosage() != null) && (conditionnement.getUniteDosage() != null)) {
                    result.append(" de " + conditionnement.getDosage()).append(EclipseConstants.SPACE).append(conditionnement.getUniteDosage());
                }
                result.append(EclipseConstants.SPACE);
                result.append("sous forme ");
                if (conditionnement.getForme().equals(FormeConditionnement.CONDITIONNEMENT_PRIMAIRE)) {
                    result.append("de ");
                }
                result.append(conditionnement.getForme());
            } else {
                result.append(conditionnement.getUniteGestion().getLibelle()).append(" contenant ").append(conditionnement.getContenance()).append(EclipseConstants.SPACE)
