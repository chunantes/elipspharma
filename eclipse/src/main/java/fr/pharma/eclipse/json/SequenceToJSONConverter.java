...
public class SequenceToJSONConverter {
    ...
    public JSONObject convert(final Sequence sequence,
        ...
        try {
            ...
            object.put("seriesName", sequence.getNom());

            final Calendar dateDebut = this.timeHelper.convertTime(date, sequence.getDebut());
            final Calendar dateFin = this.timeHelper.convertTime(date, sequence.getFin());
            dateFin.add(Calendar.DAY_OF_MONTH, 1);

            object.put("debut", this.convertDateToJSON(dateDebut));
            object.put("fin", this.convertDateToJSON(dateFin));
            object.put("idSequence", sequence.getId());
        } catch (final JSONException e) {
            throw new CommonException("Impossible de convertir la séquence.", e);
        }
    ...
    /**
     * Méthode en charge de convertir une date en Date format JSON utilisée par
     * la page de génération du graph de design.
     * @param date La date.
     * @return L'objet JSON.
     */
    private JSONObject convertDateToJSON(final Calendar date) {
        final JSONObject object = new JSONObject();
        try {
            object.put("annee", date.get(Calendar.YEAR));
            object.put("mois", date.get(Calendar.MONTH));
            object.put("jours", date.get(Calendar.DAY_OF_MONTH));
        } catch (final JSONException e) {
            throw new CommonException("Erreur de conversion de date en JSON", e);
        }
        return object;
    }
    ...
    public boolean support(final Sequence sequence) {
        return (sequence.getDebut().getNb() != null) && (sequence.getDebut().getUnite() != null) && (sequence.getFin().getNb() != null) && (sequence.getFin().getUnite() != null);
    }
